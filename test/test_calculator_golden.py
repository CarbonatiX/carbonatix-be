"""Golden tests for the ported RKEF calculator (hand-checked arithmetic)."""
...
def test_hydro_only_power_eliminates_eaf_emissions():
    r = calculate_emissions(**{**NOMINAL, "power_mix_captive_coal": 0.0})
    assert r.eaf_emissions == 0.0


def test_assess_surplus_and_deficit():
    r = calculate_emissions(**NOMINAL)
    surplus = assess(r, cap_tco2e=r.total_emissions + 1000)
    assert surplus.is_compliant
    deficit = assess(r, cap_tco2e=max(0.0, r.total_emissions - 1000))
    if r.total_emissions > 0:
        assert not deficit.is_compliant or deficit.cap_tco2e == r.total_emissions


def test_fraction_out_of_range_raises():
    with pytest.raises(ValueError, match="moisture_content_pct"):
        calculate_emissions(**{**NOMINAL, "moisture_content_pct": 32.0})
