from ..schemas import DocumentsResponse, DocumentBrief, DocumentCandidate
...
def upload_documents(db, company_id: str, files: list) -> DocumentsResponse:
    ...
    candidates = []
    for f in files:
        doc = create_document(db, company_id, file_id=f.filename, content_type=f.content_type or "application/octet-stream")
